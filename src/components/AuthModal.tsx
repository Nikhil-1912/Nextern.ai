  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'register') {
        ...
        // Dynamically extract proper user role
        let resolvedRole = UserRole.STUDENT;
        if (user.uid?.startsWith("NX-USR-SHORTCUT-")) {
          resolvedRole = user.uid.replace("NX-USR-SHORTCUT-", "") as UserRole;
        } else {
          const cleanEmail = (user.email || email).toLowerCase().trim();
          if (cleanEmail === "recruitment@shopee.com") resolvedRole = UserRole.RECRUITER;
          else if (cleanEmail === "admin@nextern.dev") resolvedRole = UserRole.ADMIN;
          else if (cleanEmail === "rostova@imperial.ac.uk") resolvedRole = UserRole.UNIVERSITY;
        }
